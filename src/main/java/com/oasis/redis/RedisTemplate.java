...
import com.oasis.redis.RedisExecution.TransactionAction;
...
/**
 * RedisTemplate provide operation template for jedis connection retrieve and
 * return back.
 * 
...
public class RedisTemplate {
    ...
    private RedisConnectionPool connPool;

    public RedisTemplate(RedisConnectionPool connPool) {
        this.connPool = connPool;
    }

    public RedisPool getWritePool() {
        RedisPool writePool = connPool.getMasterPool();
        if (writePool == null) {
            throw new IllegalStateException("Redis Master pool is unavailable.");
        }

        return writePool;
    }

    public RedisPool getReadPool() {
        RedisPool readPool = connPool.getSlavePool();
        if (readPool == null) {
            throw new IllegalStateException("Redis Slave pool is unavailable.");
        }
     ...
     */
    public <T> T executeWrite(RedisReturnAction<T> redisAction) throws JedisException {
        Jedis jedis = null;
        RedisPool redisPool = null;
        boolean broken = false;

        try {
            redisPool = getWritePool();
     ...
     */
    public void executeWrite(RedisAction redisAction) throws JedisException {
        Jedis jedis = null;
        RedisPool redisPool = null;
        boolean broken = false;
     ...
     */
    public <T> T executeRead(RedisReturnAction<T> redisAction) throws JedisException {
        Jedis jedis = null;
        RedisPool redisPool = null;
        boolean broken = false;

        try {
            redisPool = getReadPool();
            jedis = redisPool.getResource();
            return redisAction.execute(jedis);
        } catch (JedisException e) {
            logger.error("Redis connection error.", e);
    ...
    /**
     * Execute a call back action with result in pipeline.
     */
    public List<Object> doPiplineAndReturn(PipelineAction pipelineAction) throws JedisException {
        Jedis jedis = null;
        RedisPool redisPool = null;
        boolean broken = false;
    ...
    /**
     * Execute a call back action without result in pipeline.
     */
    public void doPipline(PipelineAction pipelineAction) throws JedisException {
        Jedis jedis = null;
        RedisPool redisPool = null;
        boolean broken = false;
    ...
    public Boolean del(final String... keys) {
        return executeWrite(new RedisReturnAction<Boolean>() {
    ...
    public Boolean exists(final String key) {
        return executeRead(new RedisReturnAction<Boolean>() {
    ...
    public String get(final String key) {
        return executeRead(new RedisReturnAction<String>() {
    ...
    public List<String> mget(final String... keys) {
        return executeRead(new RedisReturnAction<List<String>>() {
    ...
    public String getSet(final String key, final String value) {
        return executeWrite(new RedisReturnAction<String>() {
     ...
     * not "integer" types. Simply the string
     * stored at the key is parsed as a base 10 64 bit signed integer,
    ...
    public Long incr(final String key) {
        return executeWrite(new RedisReturnAction<Long>() {
            @Override
    ...
    public Double incrByFloat(final String key, final double increment) {
        return executeWrite(new RedisReturnAction<Double>() {
            @Override
    ...
    public Map<String, String> hgetAll(final String key) {
        return executeRead(new RedisReturnAction<Map<String, String>>() {
            @Override
    ...
    public Set<String> hkeys(final String key) {
        return executeRead(new RedisReturnAction<Set<String>>() {
            @Override
    ...
    public Long hlen(final String key) {
        return executeRead(new RedisReturnAction<Long>() {
            @Override
            public Long execute(Jedis jedis) {
    ...
    public Double zscore(final String key, final String member) {
        return executeRead(new RedisReturnAction<Double>() {
    ...
    public Set<Tuple> zrangeWithScores(final String key, final int start, final int end) {
        return executeRead(new RedisReturnAction<Set<Tuple>>() {
